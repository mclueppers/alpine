...
build: ## Build necessary Docker image for building packages
  @docker image build -t dobrevit-abuild -f .docker/abuild/Dockerfile .docker/abuild

run: ## Run a command in a new Docker container; make run a=[...]
  make build
  @docker container run -it --rm -v `pwd`/build:/build -v `pwd`/public:/public dobrevit-abuild $(a)
